...
public interface BaseResource<P, S, I, U> {
    ...
    @PostMapping
    ResponseEntity<ApiResponse<Void>> create(@Valid @RequestBody I request);

    @PutMapping("/{id}")
    ResponseEntity<ApiResponse<Void>> update(@Valid @RequestBody U request, @PathVariable int id);
